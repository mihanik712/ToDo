...
const ItemSelectPanelContainer = ({
	...
	const handleSortPriority = useCallback(() =>
		dispatch(toggleSort(SortField.priority, sortType === SortType.asc ? SortType.desc : SortType.asc)
	), [sortType]);

	const handleSortDate = useCallback(() =>
		dispatch(toggleSort(SortField.date, sortType === SortType.asc ? SortType.desc : SortType.asc)
	), [sortType]);

	const handleSortDeadLine = useCallback(() =>
		dispatch(toggleSort(SortField.deadLine, sortType === SortType.asc ? SortType.desc : SortType.asc)
	), [sortType]);
